...
export function getWeatherTheme(code) {
  if (code === 0)  return { bg: "linear-gradient(135deg, #FFD700, #FFA500)", accent: "#F59E0B", text: "#1A0F00", condColor: "#F59E0B",   type: "sunny"   };
  if (code <= 2)   return { bg: "linear-gradient(135deg, #87CEEB, #B0C4DE)", accent: "#60A5FA", text: "#1A0F00", condColor: "#5f87c7ff", type: "cloudy"  };
  if (code === 3)  return { bg: "linear-gradient(135deg, #9CA3AF, #6B7280)", accent: "#9CA3AF", text: "#FFFFFF", condColor: "#D1D5DB",   type: "overcast"};
  if (code <= 67)  return { bg: "linear-gradient(135deg, #4A7FA5, #2C5F7A)", accent: "#3B82F6", text: "#FFFFFF", condColor: "#60A5FA",   type: "rainy"   };
  if (code <= 77)  return { bg: "linear-gradient(135deg, #E0F2FE, #BAE6FD)", accent: "#7DD3FC", text: "#1A0F00", condColor: "#38BDF8",   type: "snowy"   };
  if (code <= 99)  return { bg: "linear-gradient(135deg, #1F2937, #374151)", accent: "#818CF8", text: "#FFFFFF", condColor: "#A78BFA",   type: "storm"   };
  return           { bg: "linear-gradient(135deg, #D1FAE5, #6EE7B7)", accent: "#10B981", text: "#1A0F00", condColor: "#10B981",   type: "default" };
}
...
export function getWeatherAlert(weatherData) {
  ...
  for (let i = 1; i <= 6; i++) {
    ...
    if (rain >= 60 && currentRain < 40)
      return { icon: "🌧️", title: "Rain coming",         body: `${rain}% chance of rain around ${display}`,      tag: "Heads up", color: "blue"  };
    if (code >= 95)
      return { icon: "⛈️", title: "Storm approaching",   body: `Thunderstorm expected around ${display}`,        tag: "Warning",  color: "red"   };
    if (tempDrop >= 5)
      return { icon: "🥶", title: "Temperature dropping", body: `Drops ${Math.round(tempDrop)}° colder by ${display}`, tag: "Heads up", color: "blue" };
    if (wind > 40 && (weatherData.hourly.windspeed_10m?.[startIndex] ?? 0) < 25)
      return { icon: "💨", title: "Strong winds ahead",  body: `Wind picks up significantly around ${display}`,  tag: "Heads up", color: "amber" };
    if (code >= 71 && code <= 77 && currentCode < 71)
      return { icon: "❄️", title: "Snow expected",       body: `Snow likely around ${display}`,                  tag: "Warning",  color: "blue"  };
  }

  return { icon: "✅", title: "Weather looks stable", body: "No significant changes in the next 6 hours", tag: "All clear", color: "green" };
}

// ── Activity suggestions: re-evaluate based on ACTUAL weather ─────────────
// If weather is rainy/stormy, filter out outdoor activities
const OUTDOOR_ACTIVITIES = new Set(["running", "cycling", "walking", "picnic", "sports", "outdoor_work"]);
